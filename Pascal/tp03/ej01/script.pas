...
begin
	...
	writeln('8: baja fisica de un telefono');
end;
...
procedure opcion8();
var
	bin: archivo_celurares;
	nbin: String;
	dato, ultimoRegistro: celulares;
	cod: integer;
begin
	writeln('Ingrese nombre del binario: ');
	readln(nbin);
	...
	seek(bin, fileSize(bin) - 1);
	leerBinario(bin, ultimoRegistro);
	
	reset(bin);

	writeln('Ingrese codigo del celular a eliminar: ');
	readln(cod);
	leerBinario(bin, dato);


	if (cod = ultimoRegistro.cod) then begin
		seek(bin, fileSize(bin) - 1);
		truncate(bin);
	end
	else begin
		while (dato.cod <> valor_alto) and (cod <> dato.cod) do begin
			leerBinario(bin, dato);
		end;
		if (cod = dato.cod) then begin
			seek(bin, filePos(bin) - 1);
			write(bin,ultimoRegistro);
			seek(bin, fileSize(bin) - 1);
			truncate(bin);
		end;
	end;
...
Var
	...
	nomText: String;
Begin
	nomText:= 'celulares.txt';
	assign(carga, nomText);
	opcion := 9999;
	while (opcion <> 0) do
	begin
		menu();
